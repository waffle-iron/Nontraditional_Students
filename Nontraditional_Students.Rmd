...
Now I want to see if there is a particular order that is relaible for all the students.
Hypothesis: Dual credit consistently comes before non-dual credit undergrad.
that comes before Grad or Prof Practice.
I'm not sure yet if I want to collapse those two, but I don't care about them right now.

```{r}
ordering_frame <- data_processed_alpha %>%
  group_by(CurrentCSN, is_undergrad, is_grad, is_prof_practice, is_dual_credit) %>%
  summarise(begin = min(TermKey), end = max(TermKey)) %>%
  ungroup() %>%
  group_by(CurrentCSN) %>%
  summarise(
    dc_beg = max(is_dual_credit * begin),
    dc_end = max(is_dual_credit * end),
    ug_beg = max(is_undergrad * begin),
    ug_end = max(is_undergrad * end),
    dc_before_ug = dc_end < ug_beg
    )
```
